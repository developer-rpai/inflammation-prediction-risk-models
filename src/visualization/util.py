...
def get_first_inflammation_appearance_entry(df):
    '''
    Calculates the first entry in the dataframe where each patient has inflammation.
    ...
    -------
    pandas.DataFrame
        Dataframe with the first entry in the dataframe where each patient has inflammation
    '''
    first_inflammation_appearance = df[df['label']==1].groupby('icustay_id').head(1)
    first_inflammation_appearance['chart_time'] = first_inflammation_appearance['chart_time'].astype('int64')
    return first_inflammation_appearance
