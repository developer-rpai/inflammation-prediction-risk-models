...
def extract_window(data=None, static_data=None, onset_name=None, horizon=0):
    ...
    for icuid in ids:
        ...
        end = static_data[static_data['icustay_id']==icuid][onset_name].values[0] #determine end time (onset of inflammation or control onset)
...
def drop_short_series(data, case_static, control_static, min_length=7, max_length=200):
    ...
    long_cases = case_static[case_static['inflammation_onset_hour']>=min_length]['icustay_id'].values
...
def get_onset_hour(case_static, control_static):
    result = pd.DataFrame(columns=['icustay_id', 'onset_hour']) # df to return mapping icustay_id to onset_hour (true inflammation onset or matched control onset)
    #prepare case and control info such that they can be joined into same df:
    case_hour = case_static[['icustay_id', 'inflammation_onset_hour']]
...
def load_data(test_size=0.1, horizon=0, na_thres=500, variable_start_index=5, data_sources=['labs','vitals','covs'], min_length=None, max_length=None, overwrite=False, split=0, binned=False, transform=False):
    ...
    for t in ['intime','inflammation_onset']: #convert string (of times) to datetime objects
    ...
    if overwrite or not os.path.isfile(labvital_outpath):
        ...
        case_labvitals = pd.merge(case_vitals, case_labs, how='outer', left_on=['icustay_id', 'chart_time', 'subject_id', 'inflammation_target'], 
            right_on=['icustay_id', 'chart_time', 'subject_id', 'inflammation_target'], sort=True)
        ...
        case_labvitals = extract_window(data=case_labvitals, static_data=case_static, onset_name='inflammation_onset', horizon=horizon)
        ...
        control_labvitals = control_labvitals.rename(columns={'pseudo_target': 'inflammation_target'})
